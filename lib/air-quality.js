...
export function buildOaqRequestUrl(settings, signatureString) {
  const base = normalizeBaseUrl(settings.apiBaseUrl);
  ...
  // signatureString is the raw query string from the OAQ broker, e.g.:
  // "URLPrefix=...&Expires=...&KeyName=...&Signature=..."
  return `${base}/${path}?${signatureString}`;
}
